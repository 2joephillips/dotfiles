...
if [ "$1" = "--update" ]; then
    ...
    #NON COMPOSED COVER TEMP PATH
    NON_RESIZED_COVER="/tmp/SPOTIFY_NON_RESIZED.png"
    
    #COMPOSED COVER TEMP PATH
    RESIZED_COVER="/tmp/SPOTIFY_RESIZED.png"

    NEW_COVER=$(playerctl metadata mpris:artUrl)

    wget -4 -O "$NON_RESIZED_COVER" $NEW_COVER
    dunstify "Now Playing" "`playerctl metadata --format "<b>{{artist}}</b> :<br/> {{ album }} - {{ title }}"`" -i "/tmp/SPOTIFY_NON_RESIZED.png"
...
if [ "$output" = "null" ]; then
    ...
    pb=$(/usr/bin/spt playback --format "%s %a: %t" 2>/dev/null || echo "null")
